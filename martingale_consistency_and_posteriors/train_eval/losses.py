...
# (https://github.com/sandylaker/ib-edl) — see arXiv:2502.06351 for citations
# of the underlying CE / sum-of-squares Bayes-risk and Dirichlet-KL losses.
...
class CEBayesRiskLoss(nn.Module):
    """Bayes-risk cross-entropy under a Dirichlet evidential posterior."""
...
def smooth_labels(labels: torch.Tensor, smooth: float) -> torch.Tensor:
    """One-hot label smoothing: shift mass ``smooth`` from the target uniformly to the rest."""
...
class SSBayesRiskLoss(nn.Module):
    """Sum-of-squares Bayes-risk loss with optional label smoothing and IB info penalty."""
...
class KLDivergenceLoss(nn.Module):
    """KL between the predicted Dirichlet (with target evidence removed) and a uniform prior."""
