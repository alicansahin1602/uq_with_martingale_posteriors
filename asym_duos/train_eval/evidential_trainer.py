# From the IB-EDL implementation
# (https://github.com/sandylaker/ib-edl). Method: "Calibrating LLMs with
# Information-Theoretic Evidential Deep Learning" (arXiv:2502.06351).
...
class EvidentialTrainer(Trainer):
    """HF Trainer for IB-EDL fine-tuning of causal LMs on multiple-choice QA.

    Predicts positive evidence over the answer-token vocabulary, optionally
    perturbing it through a variational information bottleneck, and trains
    against Dirichlet-style evidential losses. ``target_ids`` selects which
    output token positions correspond to the answer classes.
    """
    ...
    def compute_loss(
        ...
        """Compute Bayesian-risk + KL evidential loss; optionally return logits/uncertainties."""
    ...
    def apply_ib(self, logits: torch.Tensor,
        ...
        """Sample noisy evidences via the variational IB and return the KL info loss."""
...
class UpdateRegWeightCallback(TrainerCallback):
    """Linearly ramps the KL regularization weight from 0 to ``final_reg_weight`` after ``start_epoch``."""

    def __init__(self, start_epoch: float = 0.25, final_reg_weight: float = 1.0) -> None:
        super().__init__()
